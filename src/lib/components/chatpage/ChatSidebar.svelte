<script lang="ts">
  ...
  function condenseQuestion(question: string): string {
    ...
    const cleanedQuestion = question.replace(/[.,!?;:]/g, ''); // Remove common punctuation
    const tokens: string[] = cleanedQuestion.split(' ');
    ...
    // Filter out stop words and ensure tokens have a length greater than zero
    const keywords: string[] = tokens.filter(
      (token) => !stopWords.has(token.toLowerCase()) && token.length > 0,
    );
    ...
    // Sort keywords by frequency and then by length
    const sortedKeywords: string[] = Object.keys(frequencyMap).sort((a, b) => {
      if (frequencyMap[b] !== frequencyMap[a]) {
        return frequencyMap[b] - frequencyMap[a]; // Sort by frequency
      }
      return b.length - a.length; // Sort by length if frequencies are the same
    });

    // Condense to a concise version, limiting to a specific number of words
    const condensed: string = sortedKeywords.slice(0, 3).join(' ');

    // Capitalize the first letter of each word
    const capitalized = condensed
      .split(' ')
      .map((word) => word.charAt(0).toUpperCase() + word.slice(1).toLowerCase())
      .join(' ');

    // Optionally, return the original question if no keywords are found
